...
//    val im = occurrences.map(x => (for (i <- 1 to x._2 )yield (x._1,i)).toList)
//    println(im)
//    val y =im.foldLeft(List[Occurrences](Nil))((ret,x) => x::(for (i <- ret; j<-x) yield ( i ::: List(j))))
//    println(y)
      occurrences.foldRight(List[Occurrences](Nil))((x,ret) => ret:::(
        for(i<-ret ; j<- 1 to x._2) yield (x._1,j)::i
      ))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(a => (y.find(b => b._1==a._1) match {
      case Some((_,i)) => (a._1,a._2 - i)
      case None => a
    })).filter(_._2!=0)
  }
   ...
   */
  // copy from https://gist.github.com/chancila/5538544
